...
def get_data(**kwargs):
    ...
    train_data = np.load("/content/drive/MyDrive/train_data.npz")["data"]

    # Make the training data a tensor
    train_data = torch.tensor(train_data, dtype=torch.float32)

    # Load the test data
    test_data_input = np.load("/content/drive/MyDrive/test_data.npz")["data"]
    ...
    train_data_label = (train_data.clone() - 127.5) / 127.5
    train_data_input = (train_data.clone() - 127.5) / 127.5
    test_data_input = (test_data_input - 127.5) / 127.5
    train_data_input[:, :, 10:18, 10:18] = -1.0
...
def train_model(train_data_input, train_data_label, **kwargs):
    ...
    # Make Validation set (90% for testing, 10% for validation)
    ...
    for epoch in range(n_epochs):
        ...
        for x, y in tqdm(
            ...
            optimizer.zero_grad()
            output = model(x)
            loss = criterion(output, y[:, :, 10:18, 10:18])
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            for x, y in val_loader:
                x, y = x.to(device), y.to(device)
                out = model(x)
                loss = criterion(out, y[:, :, 10:18, 10:18])
                val_loss += loss.item()
    ...
    # You can plot the loss if you want
    # Set to False if you don't want to save the images
    if True:
        # Create the output directory if it doesn't exist
        if not Path("loss_graph").exists():
            Path("loss_graph").mkdir()
        ...
        plt.legend()
        plt.grid(True)
        plt.savefig(f"loss_graph/loss_plot_{n_epochs}.png")  # optional: save to file
        plt.close()
...
class Model(nn.Module):
    ...
    def __init__(self):
        ...
        self.seq = nn.Sequential(
             ...
             nn.MaxPool2d(2, stride=2, padding=0),  # 14 x 14
             nn.ReLU(),
             nn.Conv2d(32, 32, 3, padding=0),  # 12 x 12
             nn.MaxPool2d(2, stride=2, padding=0),  # 6 x 6
             nn.ReLU(),
             nn.Conv2d(32, 32, 3, padding=1), # 6 x 6
             nn.ReLU(),
             nn.Conv2d(32, 32, 3, padding=1), # 6 x 6
             nn.ReLU(),
             nn.Conv2d(32, 32, 3, padding=1), # 6 x 6
             nn.ReLU(),
             nn.ConvTranspose2d(32, 16, 3, padding=0),  # 8 x 8
             nn.ReLU(),
             nn.Conv2d(16, 1, 3, padding=1),  # 8 x 8
             nn.Tanh()
        )
...
def test_model(model, test_data_input):
    ...
    with torch.no_grad():
        ...
        for i in tqdm(
            ...
            snippet = model(test_data_input[i : i + batch_size])
            output = test_data_input[i : i + batch_size].clone()
            output[:,:,10:18,10:18] = snippet
    ...
    # Save the output
    test_data_output = (test_data_output.numpy()* 127.5) + 127.5
